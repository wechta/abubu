            	...
            	<table border="0" cellspacing="0" cellpadding="0" width="100%" class="wrapperTable1">
                	<tbody>
                    	...
                    	<tr class="class-main12">
                            ...
                            <td colspan="1" style="padding:0px 10px;"><span class="class-main14"><strong>Šifra:</strong></span></td>
							<td colspan="1" style="padding:0px 10px;"><span class="class-main14"><strong>Količina:</strong></span></td>
							...
							<td colspan="2" style="padding:0px 10px;"><span class="class-main14"><strong>Cena/gram:</strong></span></td>
					...
					foreach($ordProd as $prod) {
						...
						*/
						if($prod['web_price']) {
							...
							$prod['price']=number_format(str_replace(',','.',$prod['web_price']), 2, ',', '.');
						...
						$prodTitle = ($prod['sample'])?$prod['title'].'&nbsp;(VZOREC)':$prod['title'];						
						$prodTotal = (($ord['total']+$ord['postage']-$ord['fix_discount'])<0)?0:$ord['total']+$ord['postage']-$ord['fix_discount'];
						
						
						$returnString .=  '<tr class="class-main12">
								<td colspan="4" style="padding:0px 10px;"><span class="class-main14">'.$prodTitle.'</span></td>
								<td colspan="1" style="padding:0px 10px;"><span class="class-main14">'.$prod['code'].'</span></td>
								<td colspan="1" style="padding:0px 10px;"><span class="class-main14">'.$prod['num'].'</span></td>
								...
								<td colspan="2" style="padding:0px 10px;"><span class="class-main14">'.$this->getPrizeGram($prod).'</span></td>
        	...
        	<td colspan="2">
            	<table border="0" cellspacing="0" cellpadding="0" width="100%" class="wrapperTable1">
                	<tbody>
						...
						<tr class="class-main12">
                            <td colspan="2" style="font-size:13px;" class="formField-header"><span class="class-main14">Kupon: <strong>'.number_format($ord['fix_discount'], 2, ',', '.').'€</strong></span></td>
                        </tr>
						<tr class="class-main12">
                            <td colspan="2" style="font-size:13px;" class="formField-header"><span class="class-main14">Šifra kupona: <strong>'.$ord['discount_code'].'</strong></span></td>
                        </tr>						
                        <tr class="class-main12">
                            <td colspan="2" style="font-size:13px;" class="formField-header"><span class="class-main14">Skupaj: <strong>'.number_format($prodTotal, 2, ',', '.').'€</strong></span></td>
                        </tr>
				...
				function getPrizeGram($prod) {
					$prizeGram = "/";
					if($prod['prop2']) {
						$p = $this->getProperty2($prod['prop2']);
						if($this->isInteger($p['title'])) {
							$prizeGram = number_format(($prod['price']/1.095)/$p['title'], 5, ',', '.');
						} else {
							$prizeGram = "/";
						}
					}
				...
				function updateStock($uid) {
					...
					foreach($ordProd as $prod) {
						...
						if($prod['prop2']) {
							$p = $this->getProperty2($prod['prop2']);
							$faktor = ($this->isInteger($p['title']))?$p['title']:1;
						}
						$soldStock = intval($faktor)*intval($prod['num']);
				...
				function getOrderProducts($uid) {
					$output = array();
					$queryParts['SELECT'] = "DISTINCT tx_easyshop_products.uid, tx_easyshop_products.stock_val, tx_easyshop_products.code, tx_easyshop_products.title, tx_easyshop_products.price_prop2, tx_easyshop_products.price_disc_prop2, tx_easyshop_payment_log_products_mm.num, tx_easyshop_payment_log_products_mm.prop, tx_easyshop_payment_log_products_mm.prop2, tx_easyshop_payment_log_products_mm.price, tx_easyshop_payment_log_products_mm.web_price, tx_easyshop_payment_log_products_mm.sample";
