...
// DA SAE POLJA PREDIZPOLNIJO			 
			...
			if($this->piVars['promoCode']){
				$discountVal = 0;
				$promoDiscount = $this->getPromoDiscount($this->piVars['promoCode']);
				$discountCode = $this->piVars['promoCode'];
				$discountVal = intval($promoDiscount['discount']);
				if($discountVal) {
			...
			//foreach($prodUids as $prodUid){
				...
				$singleMarkProduct['###PRODUCT_CODE###']=($product['code'])?$product['code']:'';
			...
			if($this->piVars['checkBasket']){
				...
				if(t3lib_div::_POST('tipPlacila') && t3lib_div::_POST('buyer_email')) {					
					...
					if(t3lib_div::_POST('tipPlacila') == 1) {
						...
						$logId=$this->savePaymentLog($pType,serialize($basket_val),$promoVal,$discountCode,$buyerId,$reciverId,$discountVal,$sum_total_price_no_postage, $postageLog, $def_tax_log, $sum_total_tax_other);			
...
//t3lib_div::Debug($queryParts);
	...
	function savePaymentLog($payment_type, $basket_val, $fixDiscount, $discountCode, $buyerId, $reciverId, $discountVal, $sum_total_price_no_postage, $postageLog, $def_tax_log, $sum_total_tax_other){
		...
		$insertArray['discount_code'] = $discountCode;
