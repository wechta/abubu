...
/**
 * Plugin 'Nagradne igre v13' for the 'nagradneigrev12' extension.
 *
 * @author	Peter Wechtersbach <peter@naprej.net>
 * @package	TYPO3
 * @subpackage	tx_nagradneigrev12
 */
class tx_nagradneigrev12_pi1 extends tslib_pibase {
	var $prefixId      = 'tx_nagradneigrev12_pi1';		// Same as class name
	var $scriptRelPath = 'pi1/class.tx_nagradneigrev12_pi1.php';	// Path to this script relative to the extension dir.
	var $extKey        = 'nagradneigrev12';	// The extension key.
	...
	function main($content, $conf) {
		...
		//t3lib_utility_Debug::debug($this->piVars);
			...
			elseif($this->piVars['step']==3){
				...
				$singleMark['###FB_SEND_NAME###'] = $this->piVars['ime'].$this->conf['fbSendName'];
				$singleMark['###FB_SEND_DESCRIPTION###'] = $this->conf['fbSendDescription'];
				$singleMark['###LINK_APP###'] = $this->conf['linkApp'];
			...
			elseif($this->piVars['step']==4){
				...
				$povabil = $this->pi_getRecord('tx_nagradneigrev12_sodelujoc',$this->piVars['userid'],$checkPage=0);
				...
				$singleMark['###POGOJI###'] = $this->getSplosnePogoje();
				$singleMark['###LINK_APP###'] = $this->conf['linkApp'];
				$singleMark['###WALL_TITLE###'] = $this->conf['wallNaslov'];
				$singleMark['###WALL_SUBTITLE###'] = $this->conf['wallPodnaslov'];
				$singleMark['###FB_ID###'] = $this->conf['appid'];
	...
	function insertSodelujoc($ime, $email){
		...
		$GLOBALS['TYPO3_DB']->exec_INSERTquery('tx_nagradneigrev12_sodelujoc', $fields_values);
	...
	function insertPovabljen($ime, $email, $povabil){
		...
		$GLOBALS['TYPO3_DB']->exec_INSERTquery('tx_nagradneigrev12_povabljen', $fields_values);
		return $GLOBALS['TYPO3_DB']->sql_insert_id();
	}
	function updatePovabljen($idUser, $fields_values){
		$GLOBALS['TYPO3_DB']->exec_UPDATEquery('tx_nagradneigrev12_povabljen', '  uid='.$idUser, $fields_values, FALSE);
	}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/nagradneigrev12/pi1/class.tx_nagradneigrev12_pi1.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/nagradneigrev12/pi1/class.tx_nagradneigrev12_pi1.php']);
}
